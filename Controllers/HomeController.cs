        ...
        public async Task<IActionResult> Auto()
{
    ...
    // ИСПРАВЛЕНИЕ: Загружаем автомобили вместе с историей ТО
    ...
    // ИСПРАВЛЕНИЕ: Создаем словарь с последними записями ТО для каждого автомобиля
    var maintenanceDict = cars
        .Where(c => c.MaintenanceRecords.Any())
        .ToDictionary(
            c => c.Id, 
            c => c.MaintenanceRecords.OrderByDescending(m => m.MaintenanceDate).FirstOrDefault()
        );
    ...
    var model = cars.Select(car =>
    {
        ...
        // ИСПРАВЛЕНИЕ: Используем загруженные данные из Include
        return new CarWithRentalStatusViewModel
        {
            Car = car,
            Rental = rentals.FirstOrDefault(r => r.CarId == car.Id),
            MaintenanceRecords = car.MaintenanceRecords, // Теперь данные будут загружены           
            IsRecommended = isRec,
    ...
    return View(model);
        ...
        {
            if (isBadWeather)
                return "Учитывая погоду сегодня, рекомендуем данный автомобиль.";
            if (isGoodWeather)
                return "Учитывая погоду сегодня, рекомендуем данный автомобиль.";
            if (isNeutralWeather)
                return "Учитывая погоду сегодня, рекомендуем данный автомобиль.";
